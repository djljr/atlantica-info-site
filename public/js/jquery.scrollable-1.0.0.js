/**
 * jquery.scrollable 1.0.0. Put your HTML scroll.
 * 
 ...
 *
 * Launch  : March 2008
 * Version : 1.0.0 - Wed Jan 21 2009 12:12:19 GMT-0000 (GMT+00:00)
 */
(function($) {
	...
	function Scrollable(root, conf) {   
		...
		$.extend(self, {
			...
			click: function(index, time, fn) {
				...
				var item = wrap.children().eq(index);
				var klass = conf.activeClass;			
				
				if (!item.hasClass(klass) && (index >= 0 || index < this.getSize())) {				
					wrap.children().removeClass(klass);
					item.addClass(klass);
		...
		function load() {
			...
			// item.click()
			if (conf.clickable) {
				wrap.children().each(function(index, arg) {
					var el = $(this);
					if (!el.data("set")) {
